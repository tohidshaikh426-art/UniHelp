...
def admin_chat_view(session_id):
    ...
    try:
        ...
        live_chat_response = db.client.table('live_chat').select('*')            .eq('sessionid', session_id)            .execute()
        ...
        response = db.client.table('chat_message').select('*')            .eq('sessionid', session_id)            .order('created_at', desc=False)            .execute()
...
def technician_live_chats():
    ...
    try:
        ...
        print(f"❌ Error fetching technician live chats: {e}")
        import traceback
        print(f"Traceback: {traceback.format_exc()}")
        # Return empty list instead of crashing
        return render_template('technician/live_chats.html', live_chats=[])
...
def get_new_chats():
    ...
    try:
        ...
        print(f"❌ Error in get_new_chats: {e}")
        import traceback
        print(f"Traceback: {traceback.format_exc()}")
        # Return empty list instead of crashing
        return jsonify({
