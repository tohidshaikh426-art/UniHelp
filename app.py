...
def admin_dashboard():
    ...
    # Get statistics
    total_users = conn.execute('SELECT COUNT(*) as count FROM user').fetchone()['count']
    pending_approvals = conn.execute('SELECT COUNT(*) as count FROM user WHERE isapproved = 0').fetchone()['count']
    total_tickets = conn.execute('SELECT COUNT(*) as count FROM ticket').fetchone()['count']
    open_tickets = conn.execute('SELECT COUNT(*) as count FROM ticket WHERE status = "Open"').fetchone()['count']
    in_progress = conn.execute('SELECT COUNT(*) as count FROM ticket WHERE status = "In Progress"').fetchone()['count']
    closed_tickets = conn.execute('SELECT COUNT(*) as count FROM ticket WHERE status = "Closed"').fetchone()['count']
    
    # Get recent tickets
    recent_tickets = conn.execute('''
        SELECT t.*, u.name as user_name, u.role as user_role, tech.name as technician_name
        FROM ticket t
        JOIN user u ON t.userid = u.userid
        LEFT JOIN user tech ON t.assignedto = tech.userid
...
def admin_users():
    ...
    # Get statistics
    total_users = conn.execute('SELECT COUNT(*) as count FROM user').fetchone()['count']
    admin_count = conn.execute('SELECT COUNT(*) as count FROM user WHERE role = "admin"').fetchone()['count']
    tech_count = conn.execute('SELECT COUNT(*) as count FROM user WHERE role = "technician"').fetchone()['count']
    staff_count = conn.execute('SELECT COUNT(*) as count FROM user WHERE role = "staff"').fetchone()['count']
    student_count = conn.execute('SELECT COUNT(*) as count FROM user WHERE role = "student"').fetchone()['count']
...
def approve_user(user_id):
    ...
    # Get user details before approval (to access plain password if stored, but since hashed, we'll send username and note to reset)
    ...
    conn.execute('UPDATE user SET isapproved = 1 WHERE userid = ?', (user_id,))
    conn.commit()
    conn.close()
...
def admin_tickets():
    ...
    technicians = conn.execute('SELECT * FROM user WHERE role = "technician" AND isapproved = 1').fetchall()
...
def reassign_ticket(ticket_id):
    ...
    technician = conn.execute('SELECT * FROM user WHERE userid = ? AND role = "technician" AND isapproved = 1',
                             (new_technician_id,)).fetchone()
    ...
    conn.execute('UPDATE ticket SET assignedto = ?, updatedat = CURRENT_TIMESTAMP WHERE ticketid = ?', 
...
def auto_rebalance_tickets():
    ...
    unassigned_tickets = conn.execute('''
        SELECT ticketid, category FROM ticket 
    ...
    for ticket in unassigned_tickets:
        technician = auto_assign_ticket(conn, ticket['ticketid'], ticket['category'])
        if technician:
            reassigned_count += 1
...
def assign_ticket(ticket_id):
    ...
    conn = get_db_connection()
    conn.execute('UPDATE ticket SET assignedto = ?, status = "In Progress", updatedat = CURRENT_TIMESTAMP WHERE ticketid = ?', 
...
def update_ticket(ticket_id):
    ...
    ticket = conn.execute('SELECT * FROM ticket WHERE ticketid = ? AND assignedto = ?',
                         (ticket_id, session.get('user_id'))).fetchone()
    ...
    # Check if status is changing to resolved/closed
    is_resolution = status in ['Resolved', 'Closed'] and ticket['status'] != status
    ...
    if is_resolution:
        update_fields.extend(['resolvedat = CURRENT_TIMESTAMP', 'resolvedby = ?'])
        update_values.extend([session.get('user_id')])

    if resolution_notes:
        update_fields.append('resolution_notes = ?')
        update_values.append(resolution_notes)

    if satisfaction_rating:
        update_fields.append('satisfaction_rating = ?')
        update_values.append(int(satisfaction_rating))

    update_values.append(ticket_id)

    conn.execute(f'''
        UPDATE ticket SET {', '.join(update_fields)}
        WHERE ticketid = ?
    ''', update_values)
    ...
    conn.execute('''
        ...
        VALUES (?, ?, ?, ?, ?)
    ''', (ticket_id, session.get('user_id'), ticket['status'], status, comment or 'Status update'))

    # If resolving ticket, check for active work session and end it
    if is_resolution:
        # End any active work session for this ticket
        active_session = conn.execute('''
            SELECT * FROM technician_work_log
            WHERE technicianid = ? AND ticketid = ? AND end_time IS NULL
        ''', (session.get('user_id'), ticket_id)).fetchone()
        ...
        if active_session:
            ...
            conn.execute('''
                ...
                WHERE worklogid = ?
            ''', (hours_worked, active_session['worklogid']))
    ...
    if comment:
        conn.execute('''
            ...
            VALUES (?, ?, ?)
        ''', (comment, ticket_id, session.get('user_id')))
...
def create_ticket():
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        cursor = conn.execute('''
            INSERT INTO ticket (title, description, category, userid, filepath)
            VALUES (?, ?, ?, ?, ?)
        ''', (title, description, category, session.get('user_id'), filepath))
        
        ticket_id = cursor.lastrowid
        
        # Auto-assign ticket to available technician
        assigned_technician = auto_assign_ticket(conn, ticket_id, category)
...
def view_ticket(ticket_id):
    ...
    # Get ticket details
    ticket = conn.execute('''
        SELECT t.*, u.name as user_name, u.email as user_email, u.role as user_role, tech.name as technician_name
        FROM ticket t
        JOIN user u ON t.userid = u.userid
        LEFT JOIN user tech ON t.assignedto = tech.userid
        WHERE t.ticketid = ?
...
def add_comment(ticket_id):
    ...
    ticket = conn.execute('SELECT * FROM ticket WHERE ticketid = ?', (ticket_id,)).fetchone()
...
def profile():
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        user = conn.execute('SELECT * FROM user WHERE userid = ?', (session.get('user_id'),)).fetchone()
        
        if check_password_hash(user['passwordhash'], current_password):
            new_hash = generate_password_hash(new_password)
            conn.execute('UPDATE user SET passwordhash = ? WHERE userid = ?', 
...
def start_chat_session():
    ...
    if active_session:
        ...
        cursor = conn.execute('''
            INSERT INTO chat_session (userid, status)
            VALUES (?, 'active')
        ''', (session.get('user_id'),))
...
def send_chat_message():
    ...
    conn.execute('''
        ...
        VALUES (?, 'bot', ?, ?)
    ''', (session_id, bot_response, intent))
...
def request_technician_chat():
    ...
    # Get user info
    user = conn.execute('SELECT userid, name, role FROM user WHERE userid = ?',
                       (session.get('user_id'),)).fetchone()
    
    # Find available technician - more lenient for staff/admin
    user_role = session.get('role')
    if user_role in ['staff', 'admin']:
        # Staff get more lenient availability (10 minutes instead of 2)
        time_window = (datetime.now() - timedelta(minutes=10)).strftime('%Y-%m-%d %H:%M:%S')
    else:
        # Regular users get strict availability (2 minutes)
        time_window = (datetime.now() - timedelta(minutes=2)).strftime('%Y-%m-%d %H:%M:%S')
    ...
    if available_tech:
        ...
        # Update session
        conn.execute('UPDATE chat_session SET status = ? WHERE sessionid = ?',
            ('live_chat', session_id))
